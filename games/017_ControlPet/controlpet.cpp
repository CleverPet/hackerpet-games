...
unsigned char mgschwan_indexToLight (unsigned char index) {
...
unsigned char mgschwan_lightToIndex(unsigned char light) {
...
void commandCallback(String &msg)
{
    ...
    {
        String sound = mgschwan_findNthSubstring(msg, String(":"),0);
    ...
    {
        String light_idx = mgschwan_findNthSubstring(msg, String(":"),0);
        String yellow = mgschwan_findNthSubstring(msg, String(":"),1);
        String blue = mgschwan_findNthSubstring(msg, String(":"),2);
        ...
        dli->SetLights(mgschwan_indexToLight(light_idx.toInt()), yellow_val , blue_val , 0);
...
void loop()
{
    ...
    if (system_ready) 
    {
        ...
        if (buttons != 0)
        {
            ...
            if (millis()-last_button_sent[mgschwan_lightToIndex(buttons)] > 400) //Prevent double taps
            {
                mgschwan_sendString(String::format("@button_event:%d:;",buttons));
                last_button_sent[mgschwan_lightToIndex(buttons)]  = millis();
            }
        }
